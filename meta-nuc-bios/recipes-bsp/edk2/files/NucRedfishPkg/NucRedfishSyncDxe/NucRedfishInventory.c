  ...
  )
{
  ...
  NUC_REDFISH_PROCESSOR    *Processor;
  BOOLEAN                  HasCounts;
  ...
  *Count = 0;
  ...
  while (!EFI_ERROR (Smbios->GetNext (Smbios, &Handle, NULL, &Record, NULL))) {
    ...
    if (*Count >= Max) {
      DEBUG ((DEBUG_ERROR, "NucRedfishSync: more than %d processors, truncating\n", Max));
      break;
    }
    ...
    //
    // The raw identification registers -- on this platform the CPUID
    // signature and feature flags coreboot wrote into the field, which is
    // exactly what Redfish's ProcessorId/IdentificationRegisters is for.
    // Copied out rather than cast: the field is a struct and need not be
    // 8-byte aligned.
    //
    ...
    DEBUG ((
      DEBUG_ERROR,
      "NucRedfishSync: processor[%d] '%a' mfr='%a' model='%a' %d/%d cores %d threads %d MHz id=%a\n",
      *Count,
      Processor->Socket,
      Processor->Manufacturer,
      Processor->Model,
  ...
  )
{
  ...
  // this payload only builds for X64, so any processor it can describe is an
  // x86-64 part. Deriving it from the SMBIOS family would add a mapping table
  ...
  if (Processor->ProcessorType != NULL) {
    AppendJsonString (Body, NUC_REDFISH_JSON_MAX, "ProcessorType", Processor->ProcessorType);
  }

  AppendJsonString (Body, NUC_REDFISH_JSON_MAX, "Socket", Processor->Socket);
  AppendJsonString (Body, NUC_REDFISH_JSON_MAX, "Name", Processor->Socket);
  AppendJsonString (Body, NUC_REDFISH_JSON_MAX, "Manufacturer", Processor->Manufacturer);
  AppendJsonString (Body, NUC_REDFISH_JSON_MAX, "Model", Processor->Model);
  AppendJsonString (Body, NUC_REDFISH_JSON_MAX, "SerialNumber", Processor->SerialNumber);
  AppendJsonString (Body, NUC_REDFISH_JSON_MAX, "PartNumber", Processor->PartNumber);

  AppendJsonNumber (Body, NUC_REDFISH_JSON_MAX, "MaxSpeedMHz", Processor->MaxSpeedMhz);
  AppendJsonNumber (Body, NUC_REDFISH_JSON_MAX, "OperatingSpeedMHz", Processor->OperatingSpeedMhz);
  AppendJsonNumber (Body, NUC_REDFISH_JSON_MAX, "TotalCores", Processor->TotalCores);
  AppendJsonNumber (Body, NUC_REDFISH_JSON_MAX, "TotalEnabledCores", Processor->TotalEnabledCores);
  AppendJsonNumber (Body, NUC_REDFISH_JSON_MAX, "TotalThreads", Processor->TotalThreads);
  ...
  if (Processor->IdRegisters[0] != '\0') {
    ...
    AsciiStrCatS (Body, NUC_REDFISH_JSON_MAX, IdObject);
  }
