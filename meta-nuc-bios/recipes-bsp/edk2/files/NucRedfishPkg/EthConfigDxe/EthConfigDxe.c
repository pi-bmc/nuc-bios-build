...
{
  NUC_ETH_CONFIG  Config;
  UINTN           Size;
  EFI_STATUS      Status;

  Size   = sizeof (Config);
  Status = gRT->GetVariable (
                  NUC_ETH_CONFIG_VARIABLE_NAME,
                  &gNucEthConfigFormSetGuid,
                  NULL,
                  &Size,
                  &Config
                  );
  ...
  )
{
  ...
  Config->MacAddress[NUC_ETH_MAC_STR_SIZE - 1]    = L'\0';
  Config->Ip4Address[NUC_ETH_IP4_STR_SIZE - 1]    = L'\0';
  Config->Ip4SubnetMask[NUC_ETH_IP4_STR_SIZE - 1] = L'\0';
  Config->Ip4Gateway[NUC_ETH_IP4_STR_SIZE - 1]    = L'\0';
  Config->Ip4Dns1[NUC_ETH_IP4_STR_SIZE - 1]       = L'\0';
  Config->Ip4Dns2[NUC_ETH_IP4_STR_SIZE - 1]       = L'\0';
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  NUC_ETH_CONFIG            Config;
  CHAR16                    MacText[NUC_ETH_MAC_STR_SIZE];
  ...
  )
{
  ...
  CopyGuid (&VendorNode.Guid, &gNucEthConfigFormSetGuid);
